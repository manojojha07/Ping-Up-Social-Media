...
const StoryModel = ({ setShowModel, fetchStories }) => {
  ...
  const bgColors = ['#4f46e5', "#7c3aed", "#d11d48", "#ca8a04", "#0d9488"];
  ...
  const MAX_VIDEO_DURATION = 60; // secounds
  ...
  const handleMediaUpload = (e) => {
    ...
    if (file.type.startsWith('video')) {
      if (file.size > MAX_VIDEO_SIZE * 1024 * 1024) {
        toast.error("Video size max 50MB");
        setMedia(null);
        setPreviewUrl(null);
        return;
      }
      ...
      video.src = URL.createObjectURL(file); // ✅ only once

      video.onloadedmetadata = () => {
        if (video.duration > MAX_VIDEO_DURATION) {
          toast.error("Video duration max 1 minute");
          setMedia(null);
          setPreviewUrl(null);
        } else {
          setMedia(file);                       // ✅ store file
          setPreviewUrl(URL.createObjectURL(file)); // ✅ correct preview
          setText('');
          setMode('media');
        }
      }
  ...
  const handleCreateStory = async () => {
    ...
    if (media_type === 'text' && !text) {
      throw new Error("  enter some text and upload image :> ");
    }
    ...
    items-center justify-center'>
      <div className="w-full max-w-md">
        ...
        <div className="rounded-lg h-96 flex items-center justify-center relative"
          ...
          {mode === 'text' && (
            <textarea value={text} className='bg-transparent text-white w-full h-full p-6 text-lg
        ...
        rounded bg-gradient-to-r from-indigo-500 to-purple-600 hover:bg-indigo-600 
        hover:to-purple-800 active:scale-95 transition cursor-pointer'>
